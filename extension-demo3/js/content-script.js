				...
				if(localStorage.getItem("refreshFlag")==="false"||localStorage.getItem("refreshFlag")===null){
					...
					setTimeout(()=>{
						...
						if(method===1){
							...
							purchaseInput.value=config.purchaseNum>config.maxNum?config.maxNum:config.purchaseNum;
							console.log(`${config.numberText}框写入购买数量`);
								...
								$.post(`${server_url}detect_captcha_code`, 
									{
										...
										result=JSON.parse(result)
										detectContent.text = result.captcha_code;
										verifyInput.value = detectContent.text;
										const confirmBtn = nodeIterator(config.confirmText);
										console.log(`捕捉到${config.confirmText}按钮`);
										setTimeout(()=>{
											confirmBtn.click();
										},2000)
								});
